...
export default function YourApp() {
  ...
  const [ipParaCaptura, setIpParaCaptura] = useState('http://192.168.25.17/i_rms_data');
  const [leituraEmAndamento, setLeituraEmAndamento] = useState(false);
  const [tempoReferencia, setTempoReferencia] = useState([]);
  var [tempoCapturas, setTempoCapturas] = useState("1");
  ...
  const mudarTempoCapturas = () => {
    setTempoCapturas(tempoAtual);
    console.log(tempoCapturas, tempoAtual, 'mudar tempo');
  }
  ...
  function MudarIp() {
    ...
    return (
      <View>
        <Card>
          <Card.Content>
            <TextInput placeholder="http://192.168.25.17/i_rms_data" mode='outlined' disabled={leituraEmAndamento} onChangeText={texto => { ipAtual = texto }}></TextInput>
          </Card.Content>
        ...
        <Card.Content>
          <Card.Actions>
            ...
            <View style={{ flex: 1, alignSelf: "center", justifyContent: "center", paddingVertical: 40, alignItems: "center", flexDirection: "row"}}>
              <TextInput keyboardType='number-pad' disabled={leituraEmAndamento} placeholder="Tempo entre requisições (s)" onChangeText={texto => {tempoCapturas = texto}} ></TextInput>
            </View>
              <View style={{ flex: 1, alignSelf: "center", justifyContent: "center", paddingVertical: 40, alignItems: "center", flexDirection: "row", alignContent: "space-between"}}>
                <Text>Estado da Captura</Text>
                <Switch
                  value={leituraEmAndamento}
                  onValueChange={()=> {verificarEstado(tempoAtual)}}
                />
              </View>
