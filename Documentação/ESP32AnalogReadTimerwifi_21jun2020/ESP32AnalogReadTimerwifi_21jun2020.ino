...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    int tamanhoVetorEnvio = (7 * ADC_SAMPLES) + 2 + (11 * ADC_SAMPLES) + 8 + 1);
    ...
    {
      if (i<(ADC_SAMPLES-1))
        retorno = snprintf(&envio[posicaoPonteiro], 8, "%.3f,", buffer_adc_data[i]);
      else
        retorno = snprintf(&envio[posicaoPonteiro], 8, "%.3f", buffer_adc_data[i]);
    ...
    retorno = snprintf(&envio[posicaoPonteiro], 2, "%s", "!!");
    posicaoPonteiro += retorno;
    ...
    request->send(200, "text/plain", envio); 
  });
 ...
 server.on("/adc_data", HTTP_GET, [](AsyncWebServerRequest *request){
   ...
   int max = min + int(1/(timer0_t*60*.000001));
    ...
    int tamanhoVetorEnvio = (7 * ADC_SAMPLES) + 2 + ((11 * (max - min)) + 8 + 1);
    char envio[tamanhoVetorEnvio] = {'\0'};
    int posicaoPonteiro = 0;
    int retorno = 0;
  ...
  server.on("/i_rms", HTTP_GET, [](AsyncWebServerRequest *request){
    int tamanhoVetorEnvio = 7;
    char envio[tamanhoVetorEnvio] = {'\0'};
    int posicaoPonteiro = 0;
    snprintf(&envio[posicaoPonteiro], 7, "%.3f,", i_rms);
    //append rms current (in A)
    //send string to http client as text/plain
    request->send(200, "text/plain", envio); 
  });
  ...
  server.on("/i_rms_data", HTTP_GET, [](AsyncWebServerRequest *request){
    int tamanhoVetorEnvio = 7 * I_RMS_VEC_SIZE + 5;
    char envio[tamanhoVetorEnvio] = {'\0'};
    int posicaoPonteiro = 0;
