...
void setup() {
  ...
  server.on("/i_rms_data", handle_OnConnect_i_rms_data);
  server.begin();
...
void handle_OnConnect_i_rms_data() {
  String ptr = "";
  //append rms current vector (in A)
  for (int i=0;i<I_RMS_VEC_SIZE;i++)
  {
    ptr += String(i_rms_data[i],3);
    if (i<(I_RMS_VEC_SIZE-1))
          ptr += ",";
  }
